...
Among 3 different conditions to generate label, the first one and the
third one has the same trend that the mean values decrease, with increase of node size, while the mean
value for the second one increase slightly which can be considered as a case. Thus, we can conclude that, with node size increasing, the
classification error decreases generally. As for reason, the prime one is 
...
label and input data. Thus, the less node size can perform better under the complicated condition.
