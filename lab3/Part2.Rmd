...
Based on the table below, we can see the values and labels from manual prediction and function based prediction

```{r, eval=FALSE}
table_pred = data.frame(round(pred_vec, 3), pred_vec_label, round(pred_func, 3), pred_func_label)
colnames(table_pred) = c("manual prediction", "manual prediction label", "prediction value", "prediction label")
